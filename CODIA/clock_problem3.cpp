    ...
    for (operation_times[0] = 0; operation_times[0] <= 3; ++operation_times[0])
    {
        for (operation_times[1] = 0; operation_times[1] <= 3; ++operation_times[1])
        {
            for (operation_times[2] = 0; operation_times[2] <= 3; ++operation_times[2])
            {
                ...
                for (int m = 0; m <= 2; ++m)
                {
                    state[0] += operation_times[0] + operation_times[1];
                    state[1] += operation_times[0] + operation_times[1] + operation_times[2];
                    state[2] += operation_times[1] + operation_times[2];
                    state[3] += operation_times[0];
                    state[4] += operation_times[0] + operation_times[2];
                    state[5] += operation_times[2];
                ...
                for (int m = 3; m <= 5; ++m)
                {
                    ...
                    {
                        temp = &state[operations[m][k] - 'A'];
                        *temp += operation_times[m];
                        *temp = *temp % 4;
                    }
                    now_total_times += operation_times[m];
                    if (now_total_times >= min_total_times)
